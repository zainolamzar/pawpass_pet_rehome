...
// 🧩 Local carousel component for each cat card
function CatCarousel({ images, breed }: { images: string[]; breed: string }) {
  ...
  return (
    <div className="relative w-full h-56 bg-gray-100 flex-shrink-0">
      <img src={images[index]} alt={breed} className="w-full h-full object-cover" />
      {images.length > 1 && (
        <>
          <button
            onClick={prev}
...
export default function CatsPage() {
  ...
  useEffect(() => {
    const fetchCats = async () => {
      ...
      if (data.success) {
        setCats(data.data.filter((cat: Cat) => cat.isActive && cat.isApproved));
      }
    };
    fetchCats();
  }, []);
  ...
  const regions = useMemo(
    () => Array.from(new Set(cats.map((c) => c.location.split(",")[0].trim()))),
    [cats]
  );
            ...
            filteredCats.map((cat) => (
              <Link key={cat._id} href={`/find-your-cat/${cat.slug}`} className="group">
                <div className="bg-white border rounded-2xl shadow-md overflow-hidden cursor-pointer hover:shadow-lg transition flex flex-col h-[360px]">
                  {/* 🐾 Individual Carousel */}
                  <CatCarousel images={cat.images} breed={cat.breed} />

                  {/* Info */}
                  <div className="p-4 flex-1 flex flex-col justify-between">
                    <div>
                      <h2 className="text-xl font-bold text-[#748873] mb-1">
                        {cat.breed}
                      </h2>
                      <p className="text-sm text-gray-600 truncate">📍 {cat.location}</p>
                      <p className="text-sm text-gray-600 truncate">⚧ {cat.gender}</p>
                      <p className="text-sm text-gray-600 truncate">🎂 {cat.age} years old</p>
                    </div>
                  </div>
                </div>
              </Link>
