...
// 🐾 Each Dog card has its own independent carousel state
function DogCarousel({ images, breed }: { images: string[]; breed: string }) {
...
export default function DogsPage() {
  ...
  const regions = useMemo(
    () => Array.from(new Set(dogs.map((d) => d.location.split(",")[0].trim()))),
    [dogs]
  );
            ...
            filteredDogs.map((dog) => (
              <Link key={dog._id} href={`/find-your-dog/${dog.slug}`} className="group">
                <div className="bg-white border rounded-2xl shadow-md overflow-hidden cursor-pointer hover:shadow-lg transition flex flex-col h-[360px]">
                  {/* 🐕 Individual Carousel */}
                  <DogCarousel images={dog.images} breed={dog.breed} />
